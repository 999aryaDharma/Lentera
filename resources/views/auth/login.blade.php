        ...
        <div class="bg-gray-100 flex rounded-2xl shadow-2xl max-w-3xl p-5"">
            ...
            <div class="md:w-1/2 px-12 my-5 text-center">
                ...
                <form action="{{ route('login.proses') }}" method="POST" class="flex flex-col gap-4">
                    @csrf
                    <input class="p-2 mt-8 rounded-xl border" type="email" name="email" placeholder="Email Address">
                    @error('email')
                        <small>{{ $message }}</small>
                    @enderror
                    <div class="relative">
                        <input id="password" class="w-full p-2 rounded-xl border" type="password" name="password" placeholder="Password">
